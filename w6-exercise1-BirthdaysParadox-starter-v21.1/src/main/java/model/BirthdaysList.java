...
public class BirthdaysList {
    ...
    public int maxPeopleWithBirthdayInSameWeek() {
        int max = 0;
        for (int i = 1; i <= MAX_DAY; i++) {
            int inWeek = countBirthdaysBetween(i, i + 6);
            if (inWeek > max) {
                max = inWeek;
            }
        }
    ...
    public int findMedianBirthday() {
        ...
        for (int i = 1; i <= MAX_DAY; i++) {
            int count = countBirthdaysOn(i);
            before += count;
            if (before >= birthdays.length / 2 && after >= birthdays.length / 2) {
                return i;
            }
            after -= count; //remove the birthdays of today after the check is done, median takes the birthday of the specific day into account
    ...
    public List<Integer> findBirthdaysCoveringHalfOfThePeople() {
        ...
        for (int i = 0; i <= MAX_DAY; i++) {
            ...
            for (int j = 1; j <= MAX_DAY; j++) {
                if (half.contains(j)) continue;
                int count = countBirthdaysOn(j);
                if (count > max) {
                    max = count;
                    nextHighestBirthday = j;
                }
            }
